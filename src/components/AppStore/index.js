...
class AppStore extends Component {
  ...
  render() {
    const FilteredAppList = this.getFilterApps()
    const {activeTabItem} = this.state
    const {searchInput} = this.state

    const onChangeFilterApps = FilteredAppList.filter(eachApp =>
      ...
      <div className="app-store-app">
        ...
        <ul className="tabs-container">
          {tabsList.map(eachItem => (
            <TabItem
              TabEachItem={eachItem}
              key={eachItem.tabId}
              onClickTabItem={this.onClickTabItem}
              highlight={activeTabItem === eachItem.tabId}
            />
          ))}
        </ul>

        <ul className="apps-container">
          {onChangeFilterApps.map(eachItem => (
            <AppItem AppEachItem={eachItem} key={eachItem.appId} />
          ))}
        </ul>
      </div>
    )
